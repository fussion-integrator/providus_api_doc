...
**Webhook Logic Requirements**1. Validate X-Auth-Signature using SHA512(Client-Id:ClientSecret).
2. Confirm settlementId and accountNumber exist and are valid.
3. Return:"success" (code 00) if valid."duplicate transaction" (code 01) if settlementId already processed."rejected transaction" (code 02) if request is invalid."system failure, retry" (code 03) for internal errors.
...
`POST``https://api-staging.providusbank.com/v1/webhook`Webhook Request (From DCS)
...
**Webhook Responses**- Success:

Success:

**Success**```inline-grid min-w-full grid-cols-[auto_1fr] [count-reset:line] print:whitespace-pre-wrap
{
...
**Duplicate**```inline-grid min-w-full grid-cols-[auto_1fr] [count-reset:line] print:whitespace-pre-wrap
{
  "requestSuccessful": true,
  "sessionId": "99990000554443332221",
...
**Rejected**```inline-grid min-w-full grid-cols-[auto_1fr] [count-reset:line] print:whitespace-pre-wrap
{
  "requestSuccessful": true,
  "sessionId": "99990000554443332221",
...
**System Failure**```inline-grid min-w-full grid-cols-[auto_1fr] [count-reset:line] print:whitespace-pre-wrap
{
  "requestSuccessful": true,
  "sessionId": "99990000554443332221",
